...
The Svelte MCP server carries the current Svelte 5 and SvelteKit documentation. It is
registered for Claude Code in `.mcp.json` and for Cursor in `.cursor/mcp.json`, pinned to one
`@sveltejs/mcp` version in both. Bump them together.

How to drive its tools is not restated here. The server ships those instructions itself and
every client receives them on connect, so a copy in this file is a third wording of one
contract and the first to go stale. `.claude/skills/svelte/SKILL.md` is the part that is
ours: the documentation-first loop, the component conventions, and the gates a Svelte change
has to clear.
