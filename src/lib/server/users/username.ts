 ...
 * restriction is deliberate: a name spelled with a Cyrillic о is a different
 * string that renders identically, and would impersonate another account.
 */
const ALLOWED = /^[a-z0-9][a-z0-9._-]*$/;

/**
 * The stored form of a username: NFKC first so visually-identical compatibility
 * forms collapse before comparison, then lowercase, so `Jordan` and `jordan`
