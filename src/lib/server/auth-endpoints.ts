 ...
 *
 * The session is labelled from the request's own `User-Agent` rather than from
 * anything the form asked for — `device-label.ts` says why, and reduces the
 * header to a short name built from constants before it goes anywhere near the
 * column.
 */
